 ...
 *
 * @param {Address|String} cryptoAddress - An address
 * @param {String} signatureString - A base64 encoded compact signature
 * @returns {Boolean}
 */
Message.prototype.verify = function verify(cryptoAddress, signatureString) {
  $.checkArgument(cryptoAddress);
  $.checkArgument(signatureString && _.isString(signatureString));

  if (_.isString(cryptoAddress)) {
    cryptoAddress = Address.fromString(cryptoAddress);
  }
  ...
  var signatureAddress = Address.fromPublicKey(publicKey, cryptoAddress.network);

  // check that the recovered address and specified address match
  if (cryptoAddress.toString() !== signatureAddress.toString()) {
 ...
 * @returns {String}
 */
Message.prototype.recoverPublicKey = function recoverPublicKey(cryptoAddress, signatureString) {
  $.checkArgument(cryptoAddress);
  $.checkArgument(signatureString && _.isString(signatureString));
