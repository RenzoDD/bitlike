...
function Address(data, network, type, multisigType) {
  ...
  type = type == 'legacy' ? 'pubkeyhash' : type;
  type = type == 'segwit' ? 'witnesspubkeyhash' : type;
  type = type == 'native' || type == 'mulsig' || type == 'script' ? 'scripthash' : type;
 ...
 * @private
 */
Address._classifyFromVersion = function (buffer, network) {
  var version = {};
  ...
  if (buffer.length > 21) {
    ...
    var ogNetwork = Networks.get(network);
    var pubkeyhashNetwork = Networks.get(buffer[0], 'pubkeyhash', true);
    var scripthashNetwork = Networks.get(buffer[0], 'scripthash', true);

    if (pubkeyhashNetwork.indexOf(ogNetwork) != -1)
      pubkeyhashNetwork = ogNetwork;
    else
      pubkeyhashNetwork = pubkeyhashNetwork[0];

    if (scripthashNetwork.indexOf(ogNetwork) != -1)
      scripthashNetwork = ogNetwork;
    else
      scripthashNetwork = scripthashNetwork[0];
...
/**
 * Internal function to transform an address buffer
 *
 * @param {Buffer} buffer - An instance of a hex encoded address Buffer
 * @param {string=} network - The network: 'livenet' or 'testnet'
 * @param {string=} type - The type: 'legacy', 'script' or 'segwit'
...
Address._transformBuffer = function (buffer, network, type) {
  ...
  var bufferVersion = Address._classifyFromVersion(buffer, network);
 ...
 * @param {string} type - Either 'legacy', 'segwit', or 'native'
...
/**
 * Internal function to transform an address string
 *
 * @param {string} data
 * @param {String|Network=} network - either a Network instance, 'livenet', or 'testnet'
 * @param {string=} type - The type: 'legacy', 'script' or 'segwit'
 ...
 * @param {String|Network} network - either a Network instance, 'livenet', or 'testnet'
 * @param {string} type - Either 'legacy', 'script' or 'segwit'
 * @returns {Address} A new valid and frozen instance of an Address
 */
Address.fromPublicKey = function (data, network, type) {
  type = type == 'legacy' ? 'pubkeyhash' : type;
  type = type == 'script' ? 'scripthash' : type;
  type = type == 'segwit' ? 'witnesspubkeyhash' : type;
 ...
 *
 * @param {string} str - An string of the cryptocurrency address
 * @param {String|Network=} network - either a Network instance, 'livenet', or 'testnet'
 * @param {string=} type - The type of address: 'script' or 'pubkey'
 * @returns {Address} A new valid and frozen instance of an Address
 */
 ...
 *
 * @returns {Buffer} Crypto address buffer
 */
