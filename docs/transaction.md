...
Note also that an output can be used only once. That's why there's a concept of "change address" in the DigiByte ecosystem: if an output of 10 DGB is available for me to spend, but I only need to transmit 1 DGB, I'll create a transaction with two outputs, one with 1 DGB that I want to spend, and the other with 9 DGB to a change address, so I can spend this 9 DGB with another private key that I own.
...
When serializing, the DigiByteJS library performs a series of checks. These can be disabled by providing an object to the `serialize` method with the checks that you'll like to skip.
